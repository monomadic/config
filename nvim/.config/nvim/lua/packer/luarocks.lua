...
local function luarocks_install(package, results, disp)
	return async(function()
		...
		local install_result = await(
			run_luarocks("install " .. package_name .. version .. args, disp, "luarocks-install")
		)
		if results then
	...
	return async(function()
		...
		return r:map_ok(function(data)
			...
			for _, line in ipairs(output) do
				...
				if
					(components[1] == "dependency" or components[1] == "indirect_dependency")
					and (components[2] ~= "lua")
				then
...
local function handle_command(cmd, ...)
	...
	return async(function()
		...
		return r
			...
			:map_err(function(err)
				log.error(
					fmt("Failed to %s packages %s: %s", cmd, package_names, vim.fn.escape(vim.inspect(err), '"\n'))
				)
				return err
			end)
	end)()

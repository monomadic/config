...
progress lines in the log. `Esc` asks before cancelling (it signals the whole
process group, so neuroserver is not orphaned); `↵` returns to the picker when
it finishes.
...
mux pass rewrites it as an ordinary file at the end. `neuroserver-encode` adds
those flags itself unless the codec arguments already set `-movflags`.

...
bool setSystemSymbolIcon(char* symbolName) {
  NSString* name = [[NSString alloc] initWithCString:symbolName
                                            encoding:NSUTF8StringEncoding];
  free(symbolName);
...
void setTitleFont(double size, bool bold) {
  NSArray *fontConfig = @[
    [NSNumber numberWithDouble:size],
    [NSNumber numberWithBool:bold]
  ];
  runInMainThread(@selector(setTitleFont:), fontConfig);
}

...
every enhancement preset in `config/zsh/bin/lib/topaz-presets/` that does *not*
...
Install: `setup/install/install-topaz-select-preset.sh` → `~/.local/bin`.
It calls the deployed `~/.zsh/bin/topaz-preview-frame` and
`~/.zsh/bin/topaz-encode`, so the `zsh` Dotter package must be deployed.

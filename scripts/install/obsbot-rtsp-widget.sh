...
APP_NAME="obsbot-rtsp-widget"
LABEL="${LABEL:-com.jayu.obsbot-rtsp-widget}"
...
main() {
  ...
  if [[ ! -x "$SOURCE_BINARY" ]]; then
    ...
    echo "Rebuild it with: go build -ldflags='-s -w' -o '$SOURCE_BINARY' '$DOTFILES_DIR/src/$APP_NAME'" >&2
    exit 1
  fi

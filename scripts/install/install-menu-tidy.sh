#!/usr/bin/env bash
# Build and install menu-tidy (src/menu-tidy), the single-item menu bar
...
main() {
  ...
  cargo build --release --manifest-path "$DOTFILES_DIR/src/$APP_NAME/Cargo.toml"
  ...
  install -m 0755 "$DOTFILES_DIR/src/$APP_NAME/target/release/$APP_NAME" "$binary_path"

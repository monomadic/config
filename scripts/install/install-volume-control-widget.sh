...
APP_NAME="volume-control-widget"
LABEL="${LABEL:-com.jayu.volume-control-widget}"
...
main() {
  ...
  if [[ ! -x "$SOURCE_BINARY" ]]; then
    ...
    echo "Rebuild it with: cargo build --release --manifest-path '$DOTFILES_DIR/src/$APP_NAME/Cargo.toml' && cp '$DOTFILES_DIR/src/$APP_NAME/target/release/$APP_NAME' '$SOURCE_BINARY'" >&2
    exit 1
  fi

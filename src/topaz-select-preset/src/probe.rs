...
pub struct Profile {
    /// Display geometry: a phone clip stored landscape with a 90° rotation
    /// reads as portrait here, because ffmpeg autorotates before the filter.
    pub width: u32,
    pub height: u32,
    pub fps: f64,
    pub duration: f64,
...
pub fn ffmpeg_path() -> PathBuf {
    let p = ffprobe_path().with_file_name("ffmpeg");
    if p.is_file() { p } else { PathBuf::from("ffmpeg") }
}
...
pub fn probe(file: &Path) -> Result<Profile> {
    let out = Command::new(ffprobe_path())
        .args([
            ...
            "-show_entries", "stream=width,height,r_frame_rate:stream_side_data=rotation:stream_tags=rotate:format=duration",
    ...
    for line in String::from_utf8_lossy(&out.stdout).lines() {
        ...
        match k {
            ...
            "rotation" | "TAG:rotate" => {
                if rotation == 0 {
                    rotation = v.trim().parse::<f64>().map(|r| r as i64).unwrap_or(0);
                }
            }
            "r_frame_rate" => p.fps = parse_rate(v),
            _ => {}
        }
    }
...
/// Container duration in seconds; None for a missing or unreadable file.
pub fn duration(file: &Path) -> Option<f64> {
    let out = Command::new(ffprobe_path())

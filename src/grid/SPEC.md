...
**Language:** Rust. **Install target:** `~/.local/bin/grid` via `scripts/install/install-grid.sh`.
...
1. `src/grid/` crate; `clap` arg parsing for the §3 surface (parse only, most flags unimplemented).
...
4. `scripts/install/install-grid.sh` following the existing installer pattern.

...
```bash
cargo build --release --manifest-path src/volume-control-widget/Cargo.toml
cp src/volume-control-widget/target/release/volume-control-widget vendor/bin/
scripts/install/install-volume-control-widget.sh   # ~/.local/bin + LaunchAgent
```

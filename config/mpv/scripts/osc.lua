        ...
        if (element.type == "slider") then
            ...
            if not (element.slider.tooltipF == nil) then
                ...
                if mouse_hit(element) then
                    ...
                    local has_thumb = not thumbfast.disabled and thumbfast.width ~= 0 and thumbfast.height ~= 0

                    -- The thumbnail overlay already carries the seek preview.
                    -- Keep it clean by hiding the timestamp label while thumbs are active.
                    ...
                    if has_thumb then
                        ...
                        if osd_w then
                            ...
                            local thumb_margin_y = 4 / r_h
                            local thumb_x = math.min(osd_w - thumbfast.width - thumb_margin_x, math.max(thumb_margin_x, thumb_tx / r_w - thumbfast.width / 2))
                            local thumb_y = user_opts.layout ~= "topbar" and thumb_ty / r_h - thumbfast.height - thumb_margin_y or thumb_ty / r_h + thumb_margin_y

...
local send_job = "/Users/nom/.zsh/bin/send-job"
...
-- where job-server picks it up. No hostname anywhere: mount it however you
...
-- this host. It names its source purely through job-server's $TARGET_FILE
-- (the job is <video>.job, so TARGET_FILE is the video filename) and its
-- encoder through $HOME, so it runs correctly wherever it sits beside the
-- video — here (⌘R exports TARGET_FILE and runs it in the video's directory)
-- and on any render machine (job-server sets TARGET_FILE and executes it in
-- the run folder next to the copied video, as whatever user owns the share).
local function write_job_file(plan, dir)
    local home = os.getenv("HOME") or ""
    local job_argv = {}
    for i, a in ipairs(plan.argv) do
        if i == 1 and home ~= "" and a:sub(1, #home + 1) == home .. "/" then
            job_argv[i] = "$HOME/" .. a:sub(#home + 2)
        elseif a == plan.source then
            job_argv[i] = "__JOB_SOURCE__"
        else
            job_argv[i] = a
        end
    end
    ...
    local command = shell_command(job_argv)
        :gsub("^'%$HOME/(.-)'", '"$HOME"/%1', 1)
        :gsub("'__JOB_SOURCE__'", '"$TARGET_FILE"', 1)
    ...
    if not f then
        ...
        return nil, "Could not write job file"
    end
    f:write("#!/bin/zsh\n")
    f:write("# Topaz encode job — generated by the mpv Topaz workflow\n")
    f:write("# " .. plan.preset_name .. "\n")
    f:write("# Runs beside the video it encodes; no cd, source from $TARGET_FILE.\n")
    f:write(": \"${TARGET_FILE:?run me through job-server, or set TARGET_FILE}\"\n")
...
-- ⌘R (also c): save the .job beside the video, then run it in a kitty tab and
-- close the menu. Going through the job file rather than the argv directly
-- means what runs here is the exact artifact ⌘S would ship elsewhere.
function run_job_now()
    ...
    -- The job file itself is host-agnostic and takes its source from
    -- $TARGET_FILE, so a direct run has to supply the same variable
    -- job-server would.
    local result = utils.subprocess_detached({
        args = {
            ...
            "--title", " topaz job ",
            "--",
            "/usr/bin/env", "TARGET_FILE=" .. basename(plan.source), job_path,
        },
    })
    if result == false then
        mp.osd_message("Topaz job launch failed", 2)
        mp.msg.error("Failed to launch Topaz job for: " .. plan.source)
        return
    end
...
-- file is the artifact, ready for job-server here or anywhere it is copied to.
...
-- job-server then executes it wherever that folder lives.
function send_job_file()
    ...
    local job_path, job_err = write_job_file(plan, os.getenv("TMPDIR") or "/tmp/")
    if not job_path then
        mp.osd_message(job_err, 2)
        return
    end
...
function copy_encode_command()
    ...
    close_menu("Encode command copied to clipboard")
end

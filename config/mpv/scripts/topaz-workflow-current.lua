...
local function build_encode_plan()
    ...
    return {
        ...
        -- The encode as topaz-job reads it: one key=value line per flag.
        preset = {
            preset_name = preset_name,
...
local function write_preset_file(plan, dir)
    ...
    if not f then
        mp.msg.error("Preset write failed: " .. tostring(ferr))
        return nil, "Could not write preset file"
    end
    f:write("# Topaz encode preset — generated by the mpv Topaz workflow\n")
    -- Fixed order, and only single-line values: topaz-job reads each value
    -- literally to end of line.
    for _, key in ipairs({ "preset_name", "filter_complex", "output_ext",
                           "video_args", "metadata" }) do
        local value = plan.preset[key]
...
-- Build the preset and hand it to topaz-job with `mode` ("--run" / "--save" /
-- "--send"). Returns the argv, or nil + message.
local function topaz_job_args(plan, mode, dir)
    local preset_path, preset_err = write_preset_file(plan, dir)
    if not preset_path then
        return nil, preset_err
    end
    ...
    args[#args + 1] = "--preset"
    args[#args + 1] = preset_path
    args[#args + 1] = "--"
    args[#args + 1] = plan.source
    return args
end
...
function run_job_now()
    ...
    local job_args, job_err = topaz_job_args(plan, "--run", os.getenv("TMPDIR") or "/tmp/")
...
function save_job_file()
    ...
    local result = utils.subprocess({ args = job_args, cancellable = false })
    if not result or result.status ~= 0 then
        local stderr = result and trim(result.stderr) or ""
...
-- dir, then copies the video and the job into the first reachable folder — the
-- mounted share, else iCloud, else one here — and the queue runs it wherever
-- that folder lives.
function send_job_file()
    ...
    local job_args, job_err = topaz_job_args(plan, "--send", os.getenv("TMPDIR") or "/tmp/")
    if not job_args then
        mp.osd_message(job_err, 2)
        return
    end
    ...
    local result = utils.subprocess_detached({ args = args })
    if result == false then
        mp.osd_message("Could not launch topaz-job", 2)
        mp.msg.error("topaz-job launch failed for: " .. plan.source)
        return
    end

...
--
-- The job carries no cd line and names its source by bare filename, so it runs
-- correctly wherever it sits beside the video: here (⌘R, kitty --cwd'd into the
-- video's directory) and on the render machine (job-runner executes it in the
-- share root next to the copied video). The encoder path is written
-- $HOME-relative because the remote job runs as whatever user owns the share.
local function write_job_file(plan, dir)
    ...
    for i, a in ipairs(plan.argv) do
        ...
        elseif a == plan.source then
            job_argv[i] = basename(plan.source)
        else
            job_argv[i] = a
        end
    end
    -- $HOME must survive quoting, so the encoder path is quoted by hand.
    local command = shell_command(job_argv):gsub("^'%$HOME/(.-)'", '"$HOME"/%1', 1)
    ...
    f:write("# Runs beside the video it encodes; no cd, bare source filename.\n")

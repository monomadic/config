...
--
-- The job runs cd'd into the share root next to the copied video, so it must
-- carry no cd line and refer to the source by bare filename only.
function save_job_file()
    ...
    -- Rebuild the argv for the remote side: $HOME-relative encoder path (the
    -- job runs as whatever user owns the share) and a bare source filename.
    local home = os.getenv("HOME") or ""
    local remote_argv = {}
    for i, a in ipairs(plan.argv) do
        if i == 1 and home ~= "" and a:sub(1, #home + 1) == home .. "/" then
            remote_argv[i] = "$HOME/" .. a:sub(#home + 2)
        elseif a == plan.source then
            remote_argv[i] = basename(plan.source)
        else
            remote_argv[i] = a
        end
    end
    -- $HOME must survive quoting, so the encoder path is quoted by hand.
    local command = shell_command(remote_argv):gsub("^'%$HOME/(.-)'", '"$HOME"/%1', 1)

    local tmp_dir = os.getenv("TMPDIR") or "/tmp/"
    local job_name = basename(plan.source) .. ".job"
    local job_path = utils.join_path(tmp_dir, job_name)
    ...
    f:write("# Runs in the jobs share, beside the video sent with it.\n")

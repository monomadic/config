...
topaz_enhancement_preset_rows() {
  ...
  # --- workhorses first: Detail Max is menu slot 1) ---
  ...
  print -r -- $'detail\tProteus — Sharp\tproteus-sharp\t1,2,4k\ttvai_up=model=prob-4:@SCALE@:preblur=0:noise=0.12:details=0.80:halo=0.20:blur=0.35:compression=0.50:estimate=0:grain=0.02:gsize=2:device=0:vram=0.95:instances=1\tHigh detail + visible edge bite; dehalo raised\tvideoai=[Detail] Proteus Sharp. Manual Proteus: high detail generation with real edge bite — sharpening lifted to 0.35 with dehalo raised to keep the resulting halos in check. Sits between Detail Max and the Focus Fix deblurs.'
...
topaz_preset_insights() {
  ...
  print -r -- $'proteus-sharp\tstrategy\tDetail Max with edge bite: sharpening lifted to 0.35 — enough for visible crispness — while dehalo runs high to cancel the halos that sharpening would otherwise print. The middle path between clean invention and full deblur.'
  print -r -- $'proteus-sharp\twatch\tThe first preset in the Detail group where crunch is possible — check hard edges and speculars before committing.'
  print -r -- $'proteus-sharp\tvs\tproteus-detail-max\tSame invention engine plus visible sharpening. If Detail Max looks right but reads soft at edges, step up to this.'

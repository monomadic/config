...
fzf_insert() {
  ...
  files=$(fd --strip-cwd-prefix --max-depth 1 | fzf \
    ...
    --bind 'ctrl-e:execute(${EDITOR:-hx} {1})' \
...
fzf_dirs() {
  fzf --prompt '   ' \
    ...
    --padding 3%,2% \
    --preview 'eza --icons --group-directories-first {}' \
    --bind 'ctrl-e:execute-silent(${EDITOR:-hx} {1})' \
...
editor_pick() {
  if [[ "$1" == "--help" ]]; then
    echo "Usage: $0 [file ...]"
    echo " - If arguments are given, run the configured editor with the arguments."
    echo " - If no arguments are given, launch the fzf editor picker."
    return
  fi

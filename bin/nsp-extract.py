...
def gen_header(argc, argv):
    ...
    fileNamesLengths = [
        len(os.path.basename(file)) + 1 for file in argv[1:]
    ]  # +1 for the \x00 separator
...
def mk_nsp(options, args):
    ...
    if os.path.isfile(fname):
        ...
        if not options.silent:
            print("Creating new archive '%s'" % fname)
        ...
        for name in args:
            if not options.silent:
                print("Appending '%s'" % os.path.basename(name))
...
def main(argv, ret_results=False):
    ...
    actions.add_option(
        ...
        const=1,
        dest="action",
        help="Extract all files or specified files",
    )
    actions.add_option(
        "-l",
        "--list",
        ...
        const=2,
        dest="action",
        help="List all files in archive and their sizes in bytes",
    )
    actions.add_option(
        "-a",
        "--append",
        ...
        const=3,
        dest="action",
        help="Append files specified to archive",
    )
    actions.add_option(
        "-v",
        "--version",
    ...
    parser.add_option(
        ...
        default=None,
        dest="filename",
        help="Specify archive file to operate on",
    )
    parser.add_option(
        "-o",
        "--outdir",
        ...
        default=None,
        dest="outdir",
        help="Optionally set output directory for extraction",
    )
    parser.add_option(
        "-s",
        "--silent",
    ...
    if options.action == 2:
        ...
        for n in range(len(fnames)):
            print(
                "\t%s : %s bytes" % (fnames[n].ljust(maxName), fsizes[n].rjust(maxSize))
            )
        ...
        outdir = (
            ...
            else os.path.abspath(os.path.splitext(options.filename)[0])
        )
